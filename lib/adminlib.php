    ...
    public function output_html($data, $query='') {
        ...
        foreach($allenrols as $enrol => $unused) {
            ...
            if (isset($active_enrols[$enrol])) {
                ...
                $hideshow .= "<img src=\"" . $OUTPUT->pix_url('t/hide') . "\" class=\"icon-small\" alt=\"$strdisable\" /></a>";
                $enabled = true;
                $displayname = $name;
            } else if (isset($enrols_available[$enrol])) {
                $aurl = new moodle_url($url, array('action'=>'enable', 'enrol'=>$enrol));
                $hideshow = "<a href=\"$aurl\">";
                $hideshow .= "<img src=\"" . $OUTPUT->pix_url('t/show') . "\" class=\"icon-small\" alt=\"$strenable\" /></a>";
                $enabled = false;
                $displayname = $name;
                $class = 'dimmed_text';
            } else {
            ...
            if ($enabled) {
                if ($updowncount > 1) {
                    ...
                    $updown .= "<a href=\"$aurl\">";
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('t/up') . "\" alt=\"$strup\" class=\"icon-small\" /></a>&nbsp;";
                } else {
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('spacer') . "\" class=\"icon-small\" alt=\"\" />&nbsp;";
                }
                if ($updowncount < $enrolcount) {
                    $aurl = new moodle_url($url, array('action'=>'down', 'enrol'=>$enrol));
                    $updown .= "<a href=\"$aurl\">";
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('t/down') . "\" alt=\"$strdown\" class=\"icon-small\" /></a>";
                } else {
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('spacer') . "\" class=\"icon-small\" alt=\"\" />";
                }
                ++$updowncount;
    ...
    public function output_html($data, $query='') {
        ...
        foreach ($displayauths as $auth => $name) {
            ...
            if (in_array($auth, $authsenabled)) {
                ...
                $hideshow .= "<img src=\"" . $OUTPUT->pix_url('t/hide') . "\" class=\"icon-small\" alt=\"disable\" /></a>";
                ...
                $hideshow .= "<img src=\"" . $OUTPUT->pix_url('t/show') . "\" class=\"icon-small\" alt=\"enable\" /></a>";
            ...
            if ($enabled) {
                if ($updowncount > 1) {
                    ...
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('t/up') . "\" alt=\"up\" class=\"icon-small\" /></a>&nbsp;";
                }
                else {
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('spacer') . "\" class=\"icon-small\" alt=\"\" />&nbsp;";
                }
                if ($updowncount < $authcount) {
                    ...
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('t/down') . "\" alt=\"down\" class=\"icon-small\" /></a>";
                }
                else {
                    $updown .= "<img src=\"" . $OUTPUT->pix_url('spacer') . "\" class=\"icon-small\" alt=\"\" />";
                }
        ...
        foreach ($licenses as $value) {
            ...
            if ($value->enabled == 1) {
                $hideshow = html_writer::link($url.'&action=disable&license='.$value->shortname,
                    html_writer::tag('img', '', array('src'=>$OUTPUT->pix_url('t/hide'), 'class'=>'icon-small', 'alt'=>'disable')));
            } else {
                $hideshow = html_writer::link($url.'&action=enable&license='.$value->shortname,
                    html_writer::tag('img', '', array('src'=>$OUTPUT->pix_url('t/show'), 'class'=>'icon-small', 'alt'=>'enable')));
            }

            if ($value->shortname == $CFG->sitedefaultlicense) {
                $displayname .= ' '.html_writer::tag('img', '', array('src'=>$OUTPUT->pix_url('t/locked'), 'class'=>'icon-small', 'alt'=>get_string('default'), 'title'=>get_string('default')));
                $hideshow = '';
            }
...
class admin_setting_manageformats extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        $spacer = $OUTPUT->pix_icon('spacer', '', 'moodle', array('class' => 'icon-small'));
        foreach ($formats as $format) {
            ...
            if ($format->is_enabled()) {
                ...
                if ($defaultformat === $format->name) {
                    ...
                    $hideshow = html_writer::link($url->out(false, array('action' => 'disable')),
                            $OUTPUT->pix_icon('t/hide', $txt->disable, 'moodle', array('class' => 'icon-small')));
                }
            } else {
                $strformatname = $format->displayname;
                $class = 'dimmed_text';
                $hideshow = html_writer::link($url->out(false, array('action' => 'enable')),
                    $OUTPUT->pix_icon('t/show', $txt->enable, 'moodle', array('class' => 'icon-small')));
            }
            $updown = '';
            if ($cnt) {
                $updown .= html_writer::link($url->out(false, array('action' => 'up')),
                    $OUTPUT->pix_icon('t/up', $txt->up, 'moodle', array('class' => 'icon-small'))). '';
            } else {
                $updown .= $spacer;
            }
            if ($cnt < count($formats) - 1) {
                $updown .= '&nbsp;'.html_writer::link($url->out(false, array('action' => 'down')),
                    $OUTPUT->pix_icon('t/down', $txt->down, 'moodle', array('class' => 'icon-small')));
            } else {
                $updown .= $spacer;
            }
            $cnt++;
    ...
    public function output_html($data, $query='') {
        ...
        if (!empty($repositorytypes)) {
            ...
            foreach ($repositorytypes as $i) {
                ...
                $spacer = $OUTPUT->spacer(array('height' => 15, 'width' => 15, 'class' => 'icon-small'));
